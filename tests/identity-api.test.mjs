...
before(async () => {
  ...
  await pool.query(`INSERT INTO hats (id, user_id, hat_title, username, category, rate, role, active)
    VALUES ($1, $2, 'Wedding Photographer', 'simeon', 'Film', 25000, 'talent', true)`, [ids.talentHat, ids.talent])
  await pool.query(`INSERT INTO hats (id, user_id, hat_title, username, category, rate, role, active)
    VALUES ($1, $2, 'Need a photographer', 'silabs', 'Film', 30000, 'client', true)`, [ids.clientHat, ids.client])
