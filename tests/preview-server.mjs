...
for (const role of ['client', 'talent']) {
  ...
  [ids[role], role === 'client' ? 'studio_client' : 'amara_edits', `${role}@example.test`, role === 'client'])
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/__test/session') {
      const role = url.searchParams.get('role') === 'talent' ? 'talent' : 'client'
      res.setHeader('Set-Cookie', `cw_session=${signSession({ sub: ids[role] })}; Path=/; HttpOnly; SameSite=Lax`)
      res.writeHead(302, { Location: `/messages?escrow=${ids.escrow}` }); res.end(); return
    }
