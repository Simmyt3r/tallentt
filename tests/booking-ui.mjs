...
try {
  ...
  await client.getByText(/8,000/).first().waitFor({ timeout: 5000 }).catch(async (err) => {
    console.log('Negotiation UI after counter:', await client.locator('body').innerText())
    console.log('Counter response:', await counterResponse.text())
    const thread = await client.evaluate(async () => (await fetch('/api/escrows?messages=1&escrow_id=44444444-4444-4444-8444-444444444444')).json())
    console.log('Stored pending proposal:', thread?.thread?.pending_offer)
    throw err
  })
  ...
  await client.getByRole('button', { name: 'Generate start QR', exact: true }).click()
  const startCode = await client.getByLabel('Manual code (if the talent cannot scan)').inputValue()
  assert.match(startCode, /^CT1\./)
  await talent.goto('http://127.0.0.1:5179/deals?role=talent&tab=active')
  await talent.getByLabel('Or enter the code shown by the client').fill(startCode)
  await talent.getByRole('button', { name: 'Confirm code', exact: true }).click()
  await talent.getByText('Start work · 30% payment').waitFor({ state: 'hidden' })
  await talent.goto('http://127.0.0.1:5179/messages?escrow=44444444-4444-4444-8444-444444444444')
  ...
  await client.goto('http://127.0.0.1:5179/deals?role=client&tab=active')
  await client.getByRole('button', { name: 'Generate completion QR', exact: true }).click()
  const completionCode = await client.getByLabel('Manual code (if the talent cannot scan)').inputValue()
  await talent.goto('http://127.0.0.1:5179/deals?role=talent&tab=active')
  await talent.getByLabel('Or enter the code shown by the client').fill(completionCode)
  await talent.getByRole('button', { name: 'Confirm code', exact: true }).click()
  await talent.goto('http://127.0.0.1:5179/messages?escrow=44444444-4444-4444-8444-444444444444')
