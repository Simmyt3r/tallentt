...
export default function MyBookings() {
  ...
  async function handleRelease(b) {
    if (!confirm(`Release payment for "${b.hat_title}"? This can't be undone.`)) return
    setBusyId(b.id)
    const prev = bookings
            ...
            <li key={b.id} className="flex items-center gap-4 p-4 flex-wrap">
              ...
              {b.status === 'not_funded' && (
                <button
                  ...
                  className="h-8 px-3 rounded-full bg-[#0A13E6] text-white text-[11px] font-semibold shrink-0 disabled:opacity-50"
                >
                  Fund escrow
                </button>
              )}
